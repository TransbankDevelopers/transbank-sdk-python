...
class TransactionTestCase(unittest.TestCase):
    ...
    def test_capture_transaction_error(self, mock_request_service):
        ...
        with self.assertRaises(TransactionCaptureError) as context:
            transaction.capture(self.token_mock, self.buy_order_mock, self.authorization_code_mock, self.invalid_amount)

        self.assertEqual(context.exception.args[0], responses['invalid_parameter']['error_message'])

    @patch('transbank.webpay.webpay_plus.transaction.RequestService')
    def test_increase_amount(self, mock_request_service):
        mock_request_service.put.return_value = self.mock_response
        self.mock_response.json.return_value = responses['increase_amount_response']

        transaction = Transaction()
        response = transaction.increaseAmount(self.token_mock, self.buy_order_mock, self.authorization_code_mock,
    ...
    @patch('transbank.webpay.webpay_plus.transaction.RequestService')
    def test_increase_authorization_date(self, mock_request_service):
        mock_request_service.put.return_value = self.mock_response
        self.mock_response.json.return_value = responses['increase_date_response']

        transaction = Transaction()
        response = transaction.increaseAuthorizationDate(self.token_mock, self.buy_order_mock,
    ...
    @patch('transbank.webpay.webpay_plus.transaction.RequestService')
    def test_reverse_preauthorized_amount_error(self, mock_request_service):
        mock_request_service.put.side_effect = TransactionReversePreAuthorizedAmountError(
            responses['transaction_detail_not_found']['error_message'])

        transaction = Transaction()
        with self.assertRaises(TransactionReversePreAuthorizedAmountError) as context:
            transaction.reversePreAuthorizedAmount(self.token_mock, self.buy_order_mock,
                                                   self.authorization_code_mock, self.amount_mock,
                                                   IntegrationCommerceCodes.WEBPAY_PLUS_DEFERRED)
    ...
    @patch('transbank.webpay.webpay_plus.transaction.RequestService')
    def test_deferred_capture_history_error(self, mock_request_service):
        mock_request_service.get.side_effect = TransactionDeferredCaptureHistoryError(
            responses['transaction_detail_not_found']['error_message'])

        transaction = Transaction()
        with self.assertRaises(TransactionDeferredCaptureHistoryError) as context:
            transaction.deferredCaptureHistory(self.token_mock)
