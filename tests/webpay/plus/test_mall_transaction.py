...
class TransactionMallTestCase(unittest.TestCase):
    ...
    @patch('transbank.common.request_service.requests.put')
    def test_increase_amount_mall(self, mock_put):
        self.mock_response.status_code = 200
        self.mock_response.text = json.dumps(responses['increase_amount_response'])
        mock_put.return_value = self.mock_response

        response = self.deferred_capture.increaseAmount(self.child1_buy_order, self.token_mock,
                                                        self.authorization_code_mock, self.amount1_mock,
    ...
    @patch('transbank.common.request_service.requests.put')
    def test_increase_amount_mall_exception(self, mock_put):
        self.mock_response.status_code = 422
        self.mock_response.text = json.dumps(responses['invalid_parameter'])
        mock_put.return_value = self.mock_response

        with self.assertRaises(TransactionIncreaseAmountError) as context:
            self.deferred_capture.increaseAmount(self.child1_buy_order, self.token_mock, self.authorization_code_mock,
                                                 self.invalid_amount, self.child1_commerce_code)

        self.assertTrue('Invalid value for parameter' in context.exception.message)
        self.assertEqual(context.exception.__class__, TransactionIncreaseAmountError)

    @patch('transbank.common.request_service.requests.put')
    def test_increase_authorization_date_mall_transaction(self, mock_put):
        self.mock_response.status_code = 200
        self.mock_response.text = json.dumps(responses['increase_date_response'])
        mock_put.return_value = self.mock_response

        response = self.deferred_capture.increaseAuthorizationDate(self.token_mock, self.child1_buy_order,
    ...
    @patch('transbank.common.request_service.requests.put')
    def test_reverse_preauthorized_amount_mall_exception(self, mock_put):
        self.mock_response.status_code = 400
        self.mock_response.text = json.dumps(responses['transaction_not_found'])
        mock_put.return_value = self.mock_response

        with self.assertRaises(TransactionReversePreAuthorizedAmountError) as context:
            self.deferred_capture.reversePreAuthorizedAmount(self.child1_buy_order, self.token_mock,
                                                             self.authorization_code_mock, self.amount1_mock,
                                                             self.child1_commerce_code)
    ...
    @patch('transbank.common.request_service.requests.post')
    def test_mall_deferred_capture_history_successful(self, mock_post):
        self.mock_response.status_code = 200
        self.mock_response.text = json.dumps(responses['capture_history_response'])
        mock_post.return_value = self.mock_response

        response = self.deferred_capture.deferredCaptureHistory(self.token_mock, self.child1_buy_order,
                                                                self.child1_commerce_code)
    ...
    @patch('transbank.common.request_service.requests.post')
    def test_mall_deferred_capture_history_exception(self, mock_post):
        self.mock_response.status_code = 400
        ...
        with self.assertRaises(TransactionDeferredCaptureHistoryError) as context:
            self.deferred_capture.deferredCaptureHistory(self.token_mock, self.child1_buy_order,
                                                         self.child1_commerce_code)
